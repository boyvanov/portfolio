class Validate {
  constructor() {
    ...
    this.emptyErrorMessage = "Заполните поле";
    this.incorrectEmailMessage = "Некорректный email";
  ...
  _bindEvents() {
    ...
    this.$inputEmail.addEventListener("change", event => {
      ...
      const emailStr =
        "^[-._a-zA-Za-яA-я0-9]{2,}@(?:[a-zA-Za-яА-Я0-9][-a-z-A-Z-a-я-А-Я0-9]+\\.)+[a-za-я]{2,6}$";
      ...
      if (isValidEmail && regEmail.test(this.$inputEmail.value)) {
        ...
        isValidEmail === false ||
        regEmail.test(this.$inputEmail.value) === false
      ) {
